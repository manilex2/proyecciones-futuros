...
let regTransform = urln.replace(/\/prod\/proy\/cp\/lineal\/futuros\/grafico\//, '');
...
async function renderizarCharts() {
    /*global fetch*/
    const respuesta = await fetch(`/prod/proy/cp/lineal/futuros/graficos/grafico/` + nombreIndice).then((res) => {
        return res.json();
    }).then((json) => {
