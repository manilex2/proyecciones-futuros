...
let regTransform = urln.replace(/\/prod\/proy\/cp\/nolineal\/futuros\/grafico\//, '');
    ...
    const chart = new Chart(ctx, {
        ...
        data: {
            labels: extraerdatos.map(item => {
                ...
                var hora = fechaBD.getHours() > 9? fechaBD.getHours() : `0${fechaBD.getHours()}`;
                var minutos = fechaBD.getMinutes() > 9? fechaBD.getMinutes() : `0${fechaBD.getMinutes()}`;
                var segundos = fechaBD.getSeconds() > 9? fechaBD.getSeconds() : `0${fechaBD.getSeconds()}`;
                var fechaFormateada = `${year}-${mes}-${dia} ${hora}:${minutos}:${segundos}`;
...
async function renderizarCharts() {
    /*global fetch*/
    const respuesta = await fetch(`/prod/proy/cp/nolineal/futuros/graficos/grafico/` + nombreIndice).then((res) => {
        return res.json();
    }).then((json) => {
